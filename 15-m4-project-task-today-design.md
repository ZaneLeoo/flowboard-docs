...
```text
┌───── 导航 240px ─────┬──────── 内容主列（最小 0，自然扩展）──────┬─ 任务详情 352px（选中时）─┐
│ 今天 / 焦点 / 计划等 │ 今天、日期、新建任务、筛选            │ 完成、标题、项目、日期       │
│ 工作区账户与全局导航 │ 焦点 / 计划 / 稍后 / 已完成任务行      │ 优先级、状态、描述、删除      │
│ 当前账户             │ 今日完成 x / y 项轻量摘要              │                              │
└─────────────────────┴──────────────────────────────────────┴──────────────────────────────┘
```
...
- 选择任务不会改变路由、不会打开独立页面或遮罩主工作流；在桌面宽度内，详情与任务列表保持左右并列。
- Today 不承载项目总览、跨日期统计或日历规划；这些内容分别进入项目页、统计能力和日历页。
